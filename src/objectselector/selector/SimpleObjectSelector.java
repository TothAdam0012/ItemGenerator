...
import objectselector.objectprovider.SimpleObjectProvider;
import org.jetbrains.annotations.NotNull;

import java.util.*;

public class SimpleObjectSelector<T> {
    ...
    public SimpleObjectSelector() {
        this.items = new ArrayList<>();
        this.modifiedNodes = new ArrayList<>();
        this.head = new Node();
    }

    public SimpleObjectSelector(int initialCapacity) {
    ...
    public void add(SimpleObjectProvider<T> provider, double weight) {
        items.add(new Node(provider, weight));
    }
    ...
    public T weightedSample() {
        ...
        return weightedNode(false).provider.get();
    }
    ...
    public List<T> weightedSample(int k) {
        ...
        for(int i = 0; i < k-1; i++) {
            result.add(weightedNode(true).provider.get());
        }
        result.add(weightedNode(false).provider.get());
        resetState();
        weightSum = temp;
        return result;
    }
    ...
    public class Node implements Comparable<Node> {
        ...
        public final SimpleObjectProvider<T> provider;     // the value of this node
        ...
        Node(@NotNull SimpleObjectProvider<T> provider, double weight) {
